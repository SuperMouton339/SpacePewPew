...
void applyRimLighting(inout float4 albedo, inout float3 rimLightEmission)
{
    ...
    rimLightEmission = rim * lerp(albedo, rimColor, float(0.502)) * rimStrength;
    albedo.rgb = lerp(albedo.rgb, lerp(albedo.rgb, rimColor, float(0.502)) + lerp(albedo.rgb, rimColor, float(0.502)) * rimBrighten, rim);
}
#endif
